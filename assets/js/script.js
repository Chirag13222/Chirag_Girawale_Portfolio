...
const textArray = ['Programmer', 'Front End'],
  typingDelay = 50,
  erasingDelay = 50,
  newTextDelay = 2500;
...
charLimit.innerHTML = `Limit ${CHAR_LIMIT} characters`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const attentionMessage = 'Go back to page...';
...
copy.innerHTML = `Copyright &copy 2021 - ${new Date().getFullYear()} - Chirag Girawale All Rights Reserved`;
...
const myBirthDate = new Date('2002-05-11');
...
age.forEach((item) => item.innerHTML = `${currentAge} Years old`);
...
function convertToIndianFormat() {
  const inputElement = document.getElementById('celular');
  const inputValue = inputElement.value;

  // Remove spaces and country code
  const indianFormatValue = inputValue.replace(/\s+/g, '').replace('+91', '');

  // Update the input field with the Indian format value
  inputElement.value = indianFormatValue;
}